...
def load_global_datasets() -> xr.Dataset:
    """
    SINGLETON CACHE: Loads the unified master archive plus the IFS live-forecast
    bridge files into a single RAM-resident xarray handle. Replaces previous 
    fragmented open_mfdataset calls for improved structural efficiency.
    """
...
@st.cache_resource(show_spinner=False)
def load_windowed_synoptic_arrays(anchor_date_str: str, pad_past: int = 6, pad_future: int = 6) -> xr.Dataset:
    """
    SINGLETON CACHE: Eagerly decompresses the full slider-reachable temporal 
    range into memory. This prevents dask from triggering a multi-hundred-MB 
    decompression per UI click, resulting in near-instant NumPy slicing.
    """
